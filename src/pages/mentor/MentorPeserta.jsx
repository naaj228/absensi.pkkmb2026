...
export default function MentorPeserta() {
  ...
  const getStudentDailyStatus = useCallback((student) => {
    ...
    if (student.status === 'Alpha') {
      return { label: 'Alpha', bg: 'bg-rose-50 text-rose-700 border-rose-200', dot: 'bg-rose-500', isPresent: false };
    }
    ...
    return { label: 'Belum Hadir', bg: 'bg-slate-100 text-slate-600 border-slate-200', dot: 'bg-slate-400', isPresent: false };
  }, [logs, targetDateKey]);
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-slate-100 overflow-hidden flex flex-col w-full relative z-10">
          ...
          <div className="px-5 py-2.5 bg-[#012060]/[0.02] border-b border-slate-100 flex items-center justify-between">
            <span className="text-[11px] font-bold text-slate-500 flex items-center gap-1.5">
              ...
              <span>Status Presensi Tanggal: <strong className="text-[#012060]">{formatIndonesianDate(selectedDate)}</strong></span>
            </span>
          </div>
