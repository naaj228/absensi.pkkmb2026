...
// ----------------------------------------------------
// ORMAWA SERVICE (ISOLATED SYSTEM)
// ----------------------------------------------------
export const ormawaDb = {
  async fetchAll() {
    const { data, error } = await supabase
      .from('ormawa')
      .select('*')
  ...
  async add(item) {
    const qrCode = item.qrCode || `ORM-${Date.now()}-${Math.floor(100 + Math.random() * 900)}`;
    const { data, error } = await supabase
      .from('ormawa')
      .insert({
        qr_code: qrCode,
        nama_ormawa: item.namaOrmawa,
        nama_perwakilan: item.namaPerwakilan || '',
  ...
  async update(id, fields) {
    const payload = {};
    if (fields.namaOrmawa !== undefined) payload.nama_ormawa = fields.namaOrmawa;
    if (fields.namaPerwakilan !== undefined) payload.nama_perwakilan = fields.namaPerwakilan;
    if (fields.jabatan !== undefined) payload.jabatan = fields.jabatan;
    if (fields.kontak !== undefined) payload.kontak = fields.kontak;
    if (fields.email !== undefined) payload.email = fields.email;
    if (fields.status !== undefined) payload.status = fields.status;
    if (fields.waktuHadir !== undefined) payload.waktu_hadir = fields.waktuHadir;
  ...
  async fetchSettings() {
    const { data, error } = await supabase
      .from('ormawa_settings')
      .select('*')
      .eq('id', 1)
      .maybeSingle();
    if (error) throw error;
    return data ? {
      waktuMulai: data.waktu_mulai || '07:00',
      waktuSelesai: data.waktu_selesai || '17:00',
      isActive: data.is_active !== false
    } : { waktuMulai: '07:00', waktuSelesai: '17:00', isActive: true };
  },
  ...
  async updateSettings(settings) {
    const { data, error } = await supabase
      ...
      .upsert({
        id: 1,
        waktu_mulai: settings.waktuMulai,
        waktu_selesai: settings.waktuSelesai,
  ...
  async recordScan(qrCode, dicatatOleh = 'Scanner Ormawa') {
    // 0. Check Time & Status Settings
    try {
      ...
      if (settings) {
        if (!settings.is_active) {
          return { success: false, reason: 'SESSION_DISABLED', message: 'Sesi Absensi Ormawa sedang dinonaktifkan oleh Admin.' };
        }

        // Hitung jam & menit saat ini berdasarkan zona waktu WIB (Asia/Jakarta)
        ...
        const [startH, startM] = (settings.waktu_mulai || '07:00').split(':').map(Number);
        const startMinutes = startH * 60 + startM;

        const [endH, endM] = (settings.waktu_selesai || '17:00').split(':').map(Number);
        ...
        if (currentMinutes < startMinutes) {
          return {
            success: false,
            reason: 'TOO_EARLY',
            message: `Absensi Ormawa belum dibuka. Jam operasional hari ini: ${settings.waktu_mulai} - ${settings.waktu_selesai} WIB.`
          };
        }
        ...
        if (currentMinutes > endMinutes) {
          return {
            success: false,
            reason: 'TOO_LATE',
            message: `Absensi Ormawa telah ditutup (Selesai pukul ${settings.waktu_selesai} WIB).`
          };
        }
      }
    } catch (settErr) {
      console.warn('Could not verify ormawa_settings:', settErr.message);
    }

    // 1. Check if ORMAWA exists
    ...
    if (!item) {
      return { success: false, reason: 'NOT_FOUND', message: 'Kode QR Ormawa tidak terdaftar di database.' };
    }
  ...
  async addManualLog({ ormawaId, dicatatOleh = 'Admin (Manual)', statusLog = 'Manual' }) {
    const { data: item, error: findError } = await supabase
      .from('ormawa')
      .select('*')
      .eq('id', ormawaId)
    ...
    await supabase
      .from('ormawa')
      .update({ status: 'Hadir', waktu_hadir: nowIso })
      .eq('id', ormawaId);
  ...
  async updateLog(logId, fields) {
    const payload = {};
    if (fields.statusLog !== undefined) payload.status_log = fields.statusLog;
    if (fields.dicatatOleh !== undefined) payload.dicatat_oleh = fields.dicatatOleh;
    if (fields.waktuScan !== undefined) payload.waktu_scan = fields.waktuScan;
  ...
  async deleteLog(logId, ormawaId) {
    const { error } = await supabase
      .from('absensi_ormawa')
      .delete()
      .eq('id', logId);
    ...
    if (ormawaId) {
      ...
      if (!remaining || remaining.length === 0) {
        await supabase
          .from('ormawa')
          .update({ status: 'Belum Hadir', waktu_hadir: null })
          .eq('id', ormawaId);
      }
    }
  }
